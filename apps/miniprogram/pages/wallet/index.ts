...
Page({
  data:{demo:config.mode==='demo',balance:0,rewards:{signed:false,streak:0,adCount:0,adEnabled:false,adMinSeconds:6,adUnitId:''},rules:RULES,ledgers:[] as any[],cursor:null as number|null,busy:false,loadingMore:false,showAd:false,remaining:6,adSession:'',pendingAd:false,error:'',ready:false},
  ...
  async load(){try{const s=await refresh();const l=await api('wallet.listLedgers');this.setData({balance:s.wallet.balance,rewards:s.rewards,ledgers:this.rows(l.items),cursor:l.nextCursor,pendingAd:!!wx.getStorageSync(storageKey('pending-ad')),ready:true,error:''});}catch(e){this.setData({error:(e as Error).message});}},
