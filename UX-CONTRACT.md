...
- 钱包：签到/广告/分享领奖操作互斥，按真实 signed、次数和 enabled 禁用按钮。广告每天成功领取第 1/2/3 次依次发放 200/500/1,000 分，展示三档与下次奖励，观看弹层使用开始会话的金额。提前关闭模拟广告或离页不发积分、不占次数。待确认广告使用原会话恢复，成功后刷新明细。
- 分享：钱包和好友页共用 services/shares.ts 的持久化待确认记录，发起分享不自动发奖。返回钱包后手动确认领取 100 分，不设每日次数上限；可取消尚未确认的领取。已经提交的确认发生网络异常时，保留原请求并显示“重试确认到账”，防止丢弃标识后重复领取。一次处理一笔，完成后可继续分享。此流程依赖用户自报，不显示“微信已验证分享成功”，好友或群聊均使用同一规则。
- 签到奖励按连续天数动态变化。服务端返回今日金额、下一天金额、当前七天奖励预览；页面不自行推断实际到账金额。完整奖励表从同一份服务端规则生成，断签重置为第 1 天，第 101 天起每日 500。历史已发金额不追溯重算，已有钱包不重置。精确日程见 README 奖励表及 packages/domain/index.ts。
- 好友：胜率榜与流水榜共用服务端一次读取的两份排序结果；切换保留管理状态与邀请输入，记住所选榜单。胜率按整局净盈利统计，流水按累计下注统计，均展示历史累计且排除奖励积分影响；零局用户显示未上榜。切换仅使用缓存，刷新失败保留旧数据与更新时间，并发刷新只接受最后一次请求。列表突出本人；“管理好友”显示移除按钮；取消移除无副作用。邀请失败保持令牌和字段错误。模拟好友及本机邀请限制始终明确说明。
